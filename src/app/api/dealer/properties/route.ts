...
export async function GET(req: Request) {
  try {
    ...
    const properties = await Property.find({ postedBy: session.user.id, role: "dealer" })
      .sort({ createdAt: -1 })
      .lean();
...
export async function POST(req: Request) {
  try {
    ...
    const property = await Property.create({ ...body, postedBy: session.user.id, role: "dealer" });
