  ...
  const PropertyCard: React.FC<PropertyCardProps> = ({ property }) => (
    <div className="min-w-[280px] sm:min-w-[300px] md:min-w-[320px] bg-white border rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group mb-2">
      ...
      <div className="p-3 sm:p-4">
        ...
        <h3 className="font-bold text-sm sm:text-md text-slate-800 truncate">
          {property.type || "Property"} • {property.bhk || ""}
        </h3>
        <p className="text-slate-500 text-[10px] sm:text-xs flex items-center gap-1 mb-3 sm:mb-4 italic">
          <MapPin size={12} /> {property.locality || "Unknown"},{" "}
          {property.city || ""}
        </p>
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 relative group">
      ...
      <div className="relative">
        <button
          ...
          className="absolute -left-2 sm:-left-4 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg p-2 rounded-full border opacity-0 group-hover:opacity-100 transition hidden lg:block cursor-pointer"
        >
          <ChevronLeft size={24} />
        </button>
        <div
        ...
        <button
          ...
          className="absolute -right-2 sm:-right-4 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg p-2 rounded-full border opacity-0 group-hover:opacity-100 transition hidden lg:block cursor-pointer"
        >
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <section className="relative min-h-[500px] sm:h-[650px] w-full flex items-center justify-center px-4 overflow-visible z-50 pt-20">
        ...
        <div className="relative z-[60] max-w-5xl w-full text-center text-white">
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold mb-6 sm:mb-8 drop-shadow-lg px-2">
          ...
          <div className="bg-white p-2 rounded-xl sm:rounded-2xl shadow-2xl max-w-4xl mx-auto relative overflow-visible">
            <div className="flex flex-col md:flex-row gap-1 sm:gap-2">
              ...
              <div
                className="flex-1 flex items-center px-4 py-3 cursor-pointer hover:bg-slate-50 rounded-lg text-slate-800 transition"
                onClick={() => setIsFilterOpen(!isFilterOpen)}
              >
                <SlidersHorizontal
                  className="text-blue-600 mr-2 shrink-0"
                  size={18}
                />
              ...
              <div className="absolute top-[102%] left-0 w-full bg-white rounded-xl sm:rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] p-4 sm:p-8 text-left border border-slate-200 z-[100] animate-in fade-in slide-in-from-top-2 duration-200 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6 sm:gap-y-8">
                  <div className="sm:col-span-2">
                    <label className="text-[10px] sm:text-[11px] font-black text-slate-500 uppercase tracking-wider mb-2 block">
                      1. Property Category
                    </label>
                    <div className="flex gap-1.5 sm:gap-2 flex-wrap">
                      {["Residential", "Commercial", "Land", "Industrial"].map(
                        (t) => (
                          <button
                            key={t}
                            onClick={() => {
                              setActivePropertyType(t);
                              setResType("");
                              setRooms("");
                            }}
                            className={`px-4 sm:px-6 py-2.5 border rounded-xl text-[10px] sm:text-xs font-black transition-all cursor-pointer shadow-sm ${activePropertyType === t ? "bg-blue-600 text-white border-blue-600" : "text-slate-600 bg-white border-slate-400 hover:bg-slate-50 hover:border-slate-400"}`}
                          >
                            {t}
                          </button>
                        ),
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white border border-slate-400 rounded-lg px-3 py-2.5 outline-none text-sm font-bold text-slate-800 focus:ring-2 focus:ring-blue-100 cursor-pointer shadow-sm"
                    >
                      <option>Buy</option>
                      <option>Rent</option>
                      <option>Lease</option>
                      ...
                      <div className="flex gap-1.5 sm:gap-2">
                        {["1", "2", "3", "4+"].map((r) => (
                          <button
                            key={r}
                            onClick={() => setRooms(r)}
                            className={`flex-1 h-10 border rounded-lg text-xs sm:text-sm font-bold transition flex items-center justify-center cursor-pointer shadow-sm ${rooms === r ? "bg-blue-600 text-white border-blue-600" : "bg-white border-slate-400 text-slate-600 hover:border-blue-600"}`}
                          >
                            {r}
                          </button>
                        ))}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      <input
                        ...
                        placeholder="Min"
                        className="w-full bg-white border border-slate-400 rounded-lg px-3 py-2.5 text-sm font-semibold outline-none focus:ring-2 focus:ring-blue-100 text-slate-800 shadow-sm"
                      />
                ...
                <div className="mt-6 sm:mt-3 pt-4 sm:pt-3 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                  <button
                    onClick={handleReset}
                    className="text-xs sm:text-sm font-bold text-slate-500 hover:text-red-500 flex items-center gap-1 transition cursor-pointer"
                  >
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-none px-4 py-2 rounded-lg font-bold text-xs sm:text-sm text-slate-600 hover:bg-slate-50 transition cursor-pointer border border-slate-400"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => setIsFilterOpen(false)}
                      className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-6 sm:px-8 py-2 rounded-lg font-bold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
                    >
